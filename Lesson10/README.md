...
Сначала в слое domain были созданы 2 репозитория, которые отвечают за аутентификацию пользователя. Первый - AuthRepository, который имеет 4 метода: login, loginAsGuest, register, logout. UserRoleRepository имеет один метод saveRole.

Затем была настроена регистрация пользователя через FireBase. Был создан интерфейс AuthDataSource, который имел те же методы, что и AuthRepository, для того, чтобы в слое data реализовать имплементацию данного интерфейса для подключения к FireBase (FireBaseAuthDataSource).

Аналогичный интерфейс (UserRoleDataSource) был создан для сохранения роли в SharedPreferences, а также его имплементация SharedPreferencesUserRoleDataSource.
С помощью этого интерфейса мы можем сохранить или получить роль из локального хранилища.

Далее было создано 2 объекта DTO (UserDTO, USerRoleDTO), для того, чтобы исключить зависимость слоя data от моделей из слоя domain.

В имплементации интерфейса AuthRepositoryImpl описана регистрация пользователя, его вход в систему, вход, как гостя и выход.
В имплементации интерфейса UserRoleRepositoryImpl описано сохранение и получение роли пользователя.

Также было реализовано подключение к Room и работа с замоканными данными. С помощью Room обрабатываются данные о кольцах.
Для этого был создан объект RingDAO, который отвечает за работу с таблицей, файл AppDatabase, который создает базу данных с необходимыми сущностями, сущность RingEntity, которая соответствует таблице rings в базе данных и объект для связи RingDAO и AppDataBase - DatabaseProvider, который создает базу данных.

Затем была создана имплементация интерфейса RingRepositoryImpl, в которой получаются данные из замоканных данных, и происходит связь с Room.

Далле были реализованы различные UseCase для регистрации, авторизации, входа, как гостя и выхода из приложения. 
Также был реализован вывод информации о пользователе, в том числе его роли для корректоного отображения работы приложения.

Был настроен вывод списка колец с помощью Room

<img width="371" height="805" alt="image" src="https://github.com/user-attachments/assets/63ef203c-46e5-4f49-9ae7-71d431ce2013" />

<img width="372" height="815" alt="image" src="https://github.com/user-attachments/assets/93a16e87-062b-402d-8737-660ceac48e17" />
